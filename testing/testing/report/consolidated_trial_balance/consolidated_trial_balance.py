...
def get_data(filters):
    # Fetch parent company status
    is_parent_company = frappe.db.get_value("Company", filters.company, "is_group")
    
    # Initialize dictionaries to accumulate values across all companies
    all_accounts = []
    all_gl_entries_by_account = {}
    all_opening_balances = {}

    # Determine the list of companies to process
    company_list = [filters.company]
    if is_parent_company:
        company_list = frappe.db.get_list("Company", filters={"parent_company": filters.company}, pluck="name")
    
    # Process each company in the list
    for company in company_list:
        # Create a copy of filters and update the company for the current iteration
        ...
        company_currency = company_filters.presentation_currency or erpnext.get_company_currency(company)
        ...
        opening_balances = get_opening_balances(company_filters)
        
        if company_filters.project:
            company_filters.project = [company_filters.project]
        
        set_gl_entries_by_account(
            company,
            company_filters.from_date,
            company_filters.to_date,
            min_lft,
            max_rgt,
            company_filters,
            gl_entries_by_account,
            ignore_closing_entries=not flt(company_filters.with_period_closing_entry),
            ignore_opening_entries=True,
        )
        
        calculate_values(
            accounts, gl_entries_by_account, opening_balances, company_filters.get("show_net_values")
        )
        
        accumulate_values_into_parents(accounts, accounts_by_name)

        # Accumulate GL entries across all companies
        for account, entries in gl_entries_by_account.items():
            if account not in all_gl_entries_by_account:
                all_gl_entries_by_account[account] = []
            all_gl_entries_by_account[account].extend(entries)

        # Accumulate opening balances across all companies
        for account, balance in opening_balances.items():
            if account not in all_opening_balances:
                all_opening_balances[account] = 0
            all_opening_balances[account] += balance

        # Add accounts to the accumulated list
        all_accounts.extend(accounts)

    # Ensure unique accounts by name to avoid duplication
    unique_accounts_by_name = {acc['name']: acc for acc in all_accounts}
    all_accounts = list(unique_accounts_by_name.values())

    # Final data processing
    accounts, accounts_by_name, parent_children_map = filter_accounts(all_accounts)
    calculate_values(
        accounts, all_gl_entries_by_account, all_opening_balances, filters.get("show_net_values")
    )
    accumulate_values_into_parents(accounts, accounts_by_name)
